...
const uint EncodedQuat = asuint(QuatFloat);
...
const uint CompressionBits = 10;

// 10-bit integer range (1023), which is used to encode each component's value
const uint CompressionMask = (1 << CompressionBits) - 1;
...
const uint MaxAbsQuatComponentIndex = EncodedQuat >> 30;

// decode each of the xyz components by shifting their corresponding 10 bits to the left
float4 Quat = float4(1.0, 1.0, 1.0, 1.0); // initialize the quaternion with default values
